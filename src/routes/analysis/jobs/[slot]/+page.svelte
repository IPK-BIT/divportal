...
console.log(job)
    ...
    async function updatePlot() {
        let response = await fetch('https://divportal.ipk-gatersleben.de/api/v1/files/retrieve/content', {
            method: 'POST',
            body: JSON.stringify({
                bucket: "shape",
...
<section class="flex p-4 space-x-4">
    <div class="w-1/2 min-h-10 h-fit space-y-4">
        <div class="bg-base-200 rounded-md pb-2">
            ...
            <section class="bg-base-100 m-4 rounded-lg py-2">
                ...
                <div class="px-8">
                    ...
                    <p class="space-x-1"><span class="font-semibold">Status:</span>{#if job.jobCurrentStatus==='SUCCESS'}
            ...
            <section class="bg-base-100 m-4 rounded-lg py-2">
                ...
                <div class="px-8">
                    ...
                    <p><span class="font-semibold">{parameter.parameterId}:</span> {parameter.parameterValue}</p>
                    {/each}
                    {#each job.brapiDataParameters as parameter}
                    <p class="space-x-1"><span class="font-semibold">{parameter.parameterId}:</span>
                    {#each parameter.brapiParameters as brapiParameter}
                    <span class="badge badge-sm badge-outline">{brapiParameter.brapiParameterName}={brapiParameter.brapiParameterValue}</span>
                    {/each}
                    </p>
                    {/each}
        ...
        <div class="bg-base-200 rounded-md min-h-10 h-fit pb-2">
            ...
            <div class="p-4 bg-base-100 m-4 rounded-lg">
                {#if files.length>0}
                <button class="btn btn-sm float-end">Download as Zip <Zip/></button>
                {/if}
                <ul>    
                    {#each files as file}
                    <li><button class="link" on:click={()=>{downloadFile(file.fileURL)}} >{file.fileName}</button></li>
                    {/each}
                </ul>
            </div>
        </div>
    ...
    <div class="bg-base-200 w-1/2 min-h-10 h-fit rounded-md">
        ...
        <div class="p-4">
            <select class="select select-bordered select-sm" bind:value={selectedPlot} on:change={updatePlot}>
